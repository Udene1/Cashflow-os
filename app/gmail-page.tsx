...
export default function GmailPage({connected,email,connect,checkBounces}:{connected:boolean;email:string;connect:()=>void;checkBounces:()=>void}){
 ...
 return <div className="gmailPage">
  ...
  <section className="panel"><div className="panelHead"><div><p className="eyebrow">DELIVERY CONTROL</p><h2>Email outreach</h2><p>Every system-sent email is tied to its lead. Delivery failures are checked against Gmail and reflected back into the pipeline.</p></div><button className="secondary" onClick={checkBounces}>Check Gmail delivery</button></div>{error&&<div className="dbError">{error}</div>}<div className="discoveryMeta"><span>Connected as {summary?.email||email}</span><span>{summary?.updatedAt?"Connected "+new Date(summary.updatedAt).toLocaleString():""}</span></div></section></>}
 </div>
}
