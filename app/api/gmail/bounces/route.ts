...
export async function GET() {
  try {
    ...
    return NextResponse.json({ checked: pending.length, candidates, bounced });
  } catch (e) {
    console.error("[gmail-bounces] reconciliation failed", e);
    return NextResponse.json({ error: e instanceof Error ? e.message : "Bounce reconciliation failed" }, { status: 500 });
  }
}
