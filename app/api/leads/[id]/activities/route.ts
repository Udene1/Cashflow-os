...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string}>}){
 try{await ensureSchema();const sql=getSql();const {id}=await params;const b=await req.json();const type=types.includes(b.type)?b.type:"note";const body=String(b.body??"").trim();const rawAmount=b.amount==null?null:Number(b.amount);
  ...
  if(type==="contact")await sql`UPDATE leads SET status=CASE WHEN status='Found' THEN 'Contacted' ELSE status END,last_contact=NOW(),updated_at=NOW() WHERE id=${id}`;
