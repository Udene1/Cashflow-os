...
export async function POST(req:NextRequest){
 try{await ensureSchema();const b=await req.json();const name=String(b.name??"").trim(),company=String(b.company??"").trim();
...
export async function PATCH(req:NextRequest){
 try{await ensureSchema();const b=await req.json();if(!b.id)return NextResponse.json({error:"Lead id is required"},{status:400});const status=validStatus(b.status);
