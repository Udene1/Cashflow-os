...
export async function GET(){
 try{await ensureSchema();await reconcileGmailBounces();const sql=getSql();const rows=await sql`SELECT ${sql.unsafe(select)},(SELECT COALESCE(SUM(amount),0) FROM lead_activities a WHERE a.lead_id=leads.id AND a.type='payment') AS "cashCollected" FROM leads ORDER BY CASE WHEN follow_up_at IS NOT NULL AND follow_up_at<=NOW() THEN 0 WHEN follow_up_at IS NOT NULL THEN 1 ELSE 2 END,follow_up_at ASC NULLS LAST,created_at DESC`;return NextResponse.json(rows)}
